...
class Renderer:
    """Handles rendering for the SpaceMiningEnv."""
    ...
    def render(self) -> Optional[np.ndarray]:
        ...
        if self.env.agent_inventory > 0:
            gfxdraw.filled_circle(
                ...
                agent_pos_2d[0],
                agent_pos_2d[1],
                int(5 + self.env.agent_inventory / 5),  # Larger indicator
                (200, 200, 0),
            )

        # Draw energy bar
        energy_width = int(
            40 * (self.env.agent_energy / 150.0)
        ...
        gfxdraw.aacircle(
            ...
            agent_pos_2d[0],
            agent_pos_2d[1],
            int(self.env.observation_radius * 8.0),  # scale to screen
            (100, 100, 255),
        )
        ...
        gfxdraw.aacircle(
            ...
            agent_pos_2d[0],
            agent_pos_2d[1],
            int(self.env.mining_range * 8.0),  # scale to screen
        ...
        if (
            ...
            status_text.append(
                f"MINED: {self.env.last_mining_info['extracted']:.1f} from Asteroid {self.env.last_mining_info['asteroid_id']}"
            )
            status_text.append(
                f"TOTAL MINED: {self.env.last_mining_info.get('cumulative_mining', 0.0):.1f}"
            )
